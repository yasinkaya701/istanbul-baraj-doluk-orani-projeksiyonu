...
WB_SCRIPT = ROOT / 'scripts' / 'build_istanbul_water_balance_v3.py'
OUT_DIR = ROOT / 'output' / 'istanbul_hybrid_physics_ensemble_2040'
...
def select_weight(calib: pd.DataFrame) -> tuple[float, float, pd.DataFrame]:
    ...
    wb_phys = pd.read_csv(ROOT / 'output' / 'istanbul_water_balance_v3_2040' / 'water_balance_physical_sanity_checks.csv')
...
def main() -> None:
    ...
    wb = load_module(WB_SCRIPT, 'wb_v3_for_hp_ensemble')
    ...
    scenario_w.to_csv(OUT_DIR / 'water_balance_v3_scenario_projection_monthly_2026_2040.csv', index=False)
    ...
    wb_compare = pd.read_csv(ROOT / 'output' / 'istanbul_water_balance_v3_2040' / 'water_balance_vs_benchmark_models.csv')
    wb_compare = wb_compare[wb_compare['model'] == 'water_balance_v3'][['model', 'one_step_rmse_pp', 'mean_recursive_rmse_pp', 'physics_pass_count']]
    ...
    meta.update({
        ...
        'weight_water_balance_v3': float(1.0 - weight_hybrid),
        'one_step_rmse_pp': float(one_metrics.iloc[0]['one_step_rmse_pp']),
        'mean_recursive_rmse_pp': float(recursive_metrics['rmse_pp'].mean()),
        'weight_hybrid_ridge_phys': weight_hybrid_phys,
        'weight_water_balance_v3_phys': float(1.0 - weight_hybrid_phys),
