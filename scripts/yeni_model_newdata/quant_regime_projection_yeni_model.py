...
# Allow importing shared helpers from /scripts when this file is run from /scripts/yeni_model_newdata.
_THIS_DIR = Path(__file__).resolve().parent
_PARENT_SCRIPTS_DIR = _THIS_DIR.parent
...
VAR_TR_MAP = {
    ...
    "pressure": "Basınç",
    "precip": "Yağış",
    "buhar_anlik": "Anlık Buhar Basıncı (ea)",
    "buhar_max": "Maksimum Buhar Basıncı (es_tmax)",
    "buhar_fark": "Buhar Basıncı Farkı (es_tmax-ea)",
}
...
NEWS_CATALOG_CANDIDATES = [
    Path("/Users/yasinkaya/Hackhaton/output/extreme_events/news_expanded_v3_relaxed/meteoroloji_haber_baslik_katalogu.csv"),
    Path("/Users/yasinkaya/Hackhaton/output/extreme_events/news_expanded_v2/meteoroloji_haber_baslik_katalogu.csv"),
    Path("/Users/yasinkaya/Hackhaton/output/extreme_events/news/meteoroloji_haber_baslik_katalogu.csv"),
    Path("/Users/yasinkaya/Hackhaton/output/extreme_events/news_expanded_test/meteoroloji_haber_baslik_katalogu.csv"),
    Path("/Users/yasinkaya/Hackhaton/output/extreme_events/news_expanded_loose/meteoroloji_haber_baslik_katalogu.csv"),
]
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        ...
        type=str,
        default="",
        help="Eğitimde kullanılacak alt tarih sınırı (YYYY-MM-DD). Boşsa alt sınır yok.",
    )
    p.add_argument(
        "--history-end",
        type=str,
        default="",
        help="Eğitimde kullanılacak üst tarih sınırı (YYYY-MM-DD). Boşsa üst sınır yok.",
    )
    p.add_argument(
        "--analysis-mode",
        type=str,
        ...
        help="full: projeksiyon + anomali, anomalies_only: sadece tarihsel anomali analizi.",
    )
    p.add_argument(
        "--dense-window-mode",
        type=str,
        default="auto",
        choices=["auto", "off"],
        help="auto: seyrek eski blokları temizle, off: değişkenin tüm satırlarını kullan.",
    )
    p.add_argument(
        "--news-catalog",
        type=Path,
        default=None,
        help="Meteoroloji haber başlık kataloğu CSV yolu. Boş bırakılırsa bilinen varsayılan yollar denenir.",
    )
    p.add_argument(
        "--news-window-days",
        type=int,
        default=75,
        help="Anomali tarihine göre haber eşleşme penceresi (gün).",
    )
...
def resolve_news_catalog_path(explicit_path: Path | None) -> Path | None:
    if explicit_path is not None:
        p = Path(explicit_path)
        if p.exists() and p.is_file():
            return p
        return None
    for p in NEWS_CATALOG_CANDIDATES:
        if p.exists() and p.is_file():
            return p
    return None


def load_news_catalog(path: Path | None) -> pd.DataFrame:
    cols = [
        "headline_date",
        "headline",
        "source",
    ...
    if "headline_date" not in raw.columns or "headline" not in raw.columns:
        return pd.DataFrame(columns=cols)

    out = pd.DataFrame()
    out["headline_date"] = pd.to_datetime(raw["headline_date"], errors="coerce")
    out["headline"] = raw["headline"].fillna("").astype(str).str.strip()
    out["source"] = raw["source"].fillna("").astype(str).str.strip() if "source" in raw.columns else ""
    out["url"] = raw["url"].fillna("").astype(str).str.strip() if "url" in raw.columns else ""
    out["hazard_type"] = raw["hazard_type"].fillna("").astype(str).str.strip() if "hazard_type" in raw.columns else ""
    out["scope"] = raw["scope"].fillna("").astype(str).str.strip() if "scope" in raw.columns else ""
    out["language"] = raw["language"].fillna("").astype(str).str.strip() if "language" in raw.columns else ""
    if "source_prior" in raw.columns:
        sp = pd.to_numeric(raw["source_prior"], errors="coerce").fillna(0.55)
    else:
        sp = pd.Series(np.repeat(0.55, len(out)), index=out.index, dtype=float)
    out["source_prior"] = np.clip(sp.astype(float), 0.0, 1.0)
    out["headline_text"] = out["headline"].str.lower()
    out["hazard_type_norm"] = out["hazard_type"].map(normalize_token)
    out["scope_norm"] = out["scope"].map(normalize_token)
    out = out.dropna(subset=["headline_date"])
    out = out[out["headline"].str.len() > 0].sort_values("headline_date").reset_index(drop=True)
    return out


def _hazard_relevance_score(variable: str, hazard_norm: str, headline_text: str) -> float:
    var = canonical_variable_name(variable)
    hz = normalize_token(hazard_norm)
    txt = str(headline_text).lower()

    if not hz:
        if any(k in txt for k in ["sel", "flood", "taşkın", "taskin"]):
            hz = "flood"
        elif any(k in txt for k in ["yağ", "yag", "rain", "saganak"]):
            hz = "heavy_rain"
        elif any(k in txt for k in ["dolu", "hail"]):
            hz = "hail"
        elif any(k in txt for k in ["kurak", "drought"]):
            hz = "drought"
        elif any(k in txt for k in ["attribution", "iklim", "climate"]):
            hz = "attribution"
    ...
    hz_score = float(var_map.get(var, {}).get(hz, 0.32))

    if var == "precip":
        if any(k in txt for k in ["sel", "flood", "yağ", "yag", "rain", "saganak", "dolu", "hail"]):
            hz_score = max(hz_score, 0.76)
    elif var == "humidity":
        if any(k in txt for k in ["nem", "humidity", "yağ", "yag", "kurak", "drought"]):
            hz_score = max(hz_score, 0.62)
    elif var == "temp":
        if any(k in txt for k in ["sıcak", "sicak", "heat", "kurak", "drought"]):
            hz_score = max(hz_score, 0.74)
    elif var == "pressure":
        if any(k in txt for k in ["fırtına", "firtina", "storm", "basınç", "basinc", "low pressure", "high pressure"]):
            hz_score = max(hz_score, 0.60)

    return float(np.clip(hz_score, 0.0, 1.0))


def match_news_catalog(
    ts: pd.Timestamp,
    variable: str,
    freq: str,
    news_catalog: pd.DataFrame | None,
    window_days: int,
    top_k: int = 3,
) -> pd.DataFrame:
    if news_catalog is None or news_catalog.empty:
        return pd.DataFrame()

    w = max(7, int(window_days))
    if is_yearly_freq(freq):
        w = max(w, 220)
    elif is_monthly_freq(freq):
        w = max(w, 45)
    elif is_weekly_freq(freq):
        w = max(w, 21)

    t = pd.Timestamp(ts)
    dd = (pd.to_datetime(news_catalog["headline_date"]) - t).dt.days.abs()
    cand = news_catalog.loc[dd <= w].copy()
    if cand.empty:
        return cand

    cand["days_abs"] = dd.loc[cand.index].astype(float)
    cand["day_score"] = np.clip(1.0 - (cand["days_abs"] / float(w + 1)), 0.0, 1.0)
    cand["hazard_score"] = [
        _hazard_relevance_score(variable=variable, hazard_norm=str(hz), headline_text=str(tx))
        for hz, tx in zip(cand["hazard_type_norm"], cand["headline_text"], strict=False)
    ]
    scope_map = {"istanbul": 1.00, "turkiye": 0.86}
    cand["scope_score"] = cand["scope_norm"].map(scope_map).fillna(0.72).astype(float)
    cand["source_score"] = np.clip(pd.to_numeric(cand["source_prior"], errors="coerce").fillna(0.55), 0.0, 1.0)
    cand["news_score"] = (
        0.44 * cand["hazard_score"]
        + 0.27 * cand["day_score"]
        + 0.17 * cand["source_score"]
        + 0.12 * cand["scope_score"]
    )
    cand = cand.sort_values(["news_score", "day_score", "source_score"], ascending=False)
    cand = cand.head(max(1, int(top_k))).reset_index(drop=True)
    return cand
...
def sanitize_observations_physical(obs: pd.DataFrame) -> tuple[pd.DataFrame, dict[str, int]]:
    """Apply variable-aware plausibility checks on raw observations.

    Notes:
    - Pressure archive contains mixed encoding where some records miss leading 7xx.
      For pressure values <=120 we shift by +700 to harmonize with the main scale.
    - After harmonization, hard physical bounds are enforced per variable.
    """
    out = obs.copy()
    stats: dict[str, int] = {}

    p_mask = out["variable"].astype(str).eq("pressure") & out["value"].notna()
    p_vals = pd.to_numeric(out.loc[p_mask, "value"], errors="coerce")
    p_shift_mask = p_vals <= 120.0
    if p_shift_mask.any():
        out.loc[p_mask, "value"] = np.where(p_shift_mask.values, p_vals.values + 700.0, p_vals.values)
    stats["pressure_shift_plus700_rows"] = int(p_shift_mask.sum())

    bounds: dict[str, tuple[float, float]] = {
        "humidity": (0.0, 100.0),
        "precip": (0.0, 500.0),
        "pressure": (650.0, 820.0),
        "temp": (-40.0, 55.0),
    }
    keep = pd.Series(True, index=out.index)
    for var, (lo, hi) in bounds.items():
        vmask = out["variable"].astype(str).eq(var)
        bad = vmask & ~pd.to_numeric(out["value"], errors="coerce").between(lo, hi)
        stats[f"{var}_dropped_out_of_range"] = int(bad.sum())
        keep = keep & ~bad

    out = out[keep].copy()
    out = out.dropna(subset=["timestamp", "value", "variable"]).sort_values("timestamp").reset_index(drop=True)
    stats["rows_after_physical_qc"] = int(len(out))
    return out, stats
...
def choose_frequency_plan(obs: pd.DataFrame, variable: str, ok_value: str) -> FrequencyPlan:
    ...
    # Humidity should remain monthly in this project outputs.
    if canonical_variable_name(variable) == "humidity":
...
def _dense_year_threshold(variable: str) -> int:
    var = canonical_variable_name(variable)
    if var == "humidity":
        return 250
    if var == "precip":
...
def infer_dense_window_bounds(obs: pd.DataFrame, variable: str, ok_value: str) -> tuple[pd.Timestamp | None, pd.Timestamp | None, dict[str, Any]]:
    sub = obs[obs["variable"] == variable].copy()
    if sub.empty:
        return None, None, {"threshold": 0, "run_years": 0, "run_points": 0}
    ...
    year_counts = sub.groupby(sub["timestamp"].dt.year).size().sort_index()
    if year_counts.empty:
        return None, None, {"threshold": 0, "run_years": 0, "run_points": 0}

    thr = int(_dense_year_threshold(variable))
    qualified = year_counts[year_counts >= thr]
    if qualified.empty:
        qualified = year_counts.copy()

    years = [int(y) for y in qualified.index.tolist()]
    if not years:
        y0 = int(year_counts.index.min())
        y1 = int(year_counts.index.max())
        return pd.Timestamp(year=y0, month=1, day=1), pd.Timestamp(year=y1, month=12, day=31), {
            "threshold": thr,
            "run_years": int(y1 - y0 + 1),
            "run_points": int(year_counts.sum()),
        }

    runs: list[tuple[int, int, int, int]] = []
    start = years[0]
    prev = years[0]
    pts = int(qualified.loc[prev])
    for y in years[1:]:
        if y == prev + 1:
            prev = y
            pts += int(qualified.loc[y])
        else:
            runs.append((start, prev, int(prev - start + 1), int(pts)))
            start = y
            prev = y
            pts = int(qualified.loc[y])
    runs.append((start, prev, int(prev - start + 1), int(pts)))

    runs = sorted(runs, key=lambda x: (x[2], x[3], x[1]), reverse=True)
    y0, y1, run_years, run_points = runs[0]
    return (
        pd.Timestamp(year=int(y0), month=1, day=1),
        pd.Timestamp(year=int(y1), month=12, day=31),
        {"threshold": thr, "run_years": int(run_years), "run_points": int(run_points)},
    )
...
def specific_anomaly_label(
    ...
    flag_robust: bool,
    flag_iforest: bool,
    delta_prev: float = float("nan"),
    diff_std: float = float("nan"),
) -> str:
    var = canonical_variable_name(variable)
    pos = float(residual) >= 0.0
    az = abs(float(zscore))
    arz = abs(float(robust_zscore))
    severe = bool(flag_tail) or az >= 3.0 or arz >= 3.2
    jump_up = np.isfinite(delta_prev) and np.isfinite(diff_std) and float(diff_std) > 1e-9 and float(delta_prev) > 1.5 * float(diff_std)
    jump_dn = np.isfinite(delta_prev) and np.isfinite(diff_std) and float(diff_std) > 1e-9 and float(delta_prev) < -1.5 * float(diff_std)

    if var == "precip":
        if pos:
            if severe:
                return "Aşırı yağış piki"
            if jump_up:
                return "Ani yağış sıçraması"
            return "Yağış rejimi artışı"
        if severe:
            return "Keskin yağış açığı"
        if jump_dn:
            return "Ani yağış düşüşü"
        return "Yağış rejimi zayıflaması"

    if var == "temp":
        if pos:
            if severe:
                return "Sıcak dalga atağı"
            if jump_up:
                return "Ani sıcaklık yükselişi"
            return "Sıcaklık rejimi yükselişi"
        if severe:
            return "Soğuk dalga atağı"
        if jump_dn:
            return "Ani sıcaklık düşüşü"
        return "Sıcaklık rejimi düşüşü"
    ...
    if var == "humidity":
        if pos:
            if severe:
                return "Nem piki"
            if jump_up:
                return "Ani nem artışı"
            return "Nem rejimi artışı"
        if severe:
            return "Keskin nem düşüşü"
        if jump_dn:
            return "Ani nem düşüşü"
        return "Nem rejimi zayıflaması"

    if var == "pressure":
        if pos:
            if severe:
                return "Yüksek basınç blokajı"
            if jump_up:
                return "Ani basınç yükselişi"
            return "Yüksek basınç rejimi"
        if severe:
            return "Derin alçak basınç epizodu"
        if jump_dn:
            return "Ani basınç düşüşü"
        return "Alçak basınç rejimi"

    if severe:
        return "Kritik anomali sapması"
    if flag_sigma or flag_robust or flag_iforest:
        return "Belirgin anomali sapması"
    return "Düşük şiddetli sapma"
...
def detect_anomalies(ds: pd.Series, y_true: np.ndarray, yhat: np.ndarray, var_hist: np.ndarray, z_thr: float, top_n: int, variable: str) -> pd.DataFrame:
    ...
    if len(e) >= 40:
        ...
        n_eff = max(len(e), 1)
        contam = float(np.clip(1.5 / np.sqrt(n_eff), 0.02, 0.12))
        iso = IsolationForest(
            n_estimators=300,
    ...
    delta_prev_arr = np.r_[np.nan, np.diff(y_arr)] if len(y_arr) else np.array([], dtype=float)
    diff_std = float(np.nanstd(np.diff(y_arr))) if len(y_arr) > 2 else float("nan")

    anom_type = []
    for idx, r in enumerate(out.itertuples(index=False)):
        dp = float(delta_prev_arr[idx]) if idx < len(delta_prev_arr) else float("nan")
        anom_type.append(
            specific_anomaly_label(
                variable=variable,
                residual=float(r.residual),
                zscore=float(r.zscore),
                robust_zscore=float(r.robust_zscore),
                flag_tail=bool(r.flag_tail),
                flag_sigma=bool(r.flag_sigma),
                flag_robust=bool(r.flag_robust),
                flag_iforest=bool(r.flag_iforest),
                delta_prev=dp,
                diff_std=diff_std,
            )
        )
    ...
    top_n_int = int(top_n)
    if top_n_int > 0:
        anom_only = anom_only.head(top_n_int)
...
def infer_turkey_pattern_hint(ts: pd.Timestamp, variable: str, residual: float, zscore: float) -> str:
    ...
    if m in {12, 1, 2} and z >= 1.2:
        if var == "temp" and r < 0:
            return "NAO-negatif benzeri kış sinyali (daha soğuk koşullar)"
        if var == "temp" and r > 0:
            return "NAO-pozitif benzeri kış sinyali (daha ılık koşullar)"
        if var == "precip" and r > 0:
            return "NAO-negatif benzeri sinyal (Marmara/Doğu Akdeniz yağış artışı)"
        if var == "precip" and r < 0:
            return "NAO-pozitif benzeri sinyal (yağış baskılanması)"
        if var == "pressure" and r > 0:
            return "NAO-pozitif benzeri sinyal (yüksek basınç eğilimi)"
        if var == "pressure" and r < 0:
            return "NAO-negatif benzeri sinyal (düşük basınç/siklonik eğilim)"
    ...
    if m in {6, 7, 8} and z >= 1.2:
        ...
        if var == "temp" and r > 0:
            return "Blokaj/sıcak dalga benzeri yaz sinyali"
        if var == "precip" and r < 0:
...
def enrich_anomalies_with_causes(
    ...
    variable: str,
    plan: FrequencyPlan,
    news_catalog: pd.DataFrame | None = None,
    news_window_days: int = 75,
) -> pd.DataFrame:
    if anom_df.empty:
        return anom_df
    ...
    out["local_climate_hint"] = ""
    out["news_headline_match"] = "doğrudan eşleşme yok"
    out["news_source_match"] = ""
    out["news_url_match"] = ""
    out["news_hazard_match"] = ""
    out["news_match_score"] = np.nan
    out["news_match_count"] = 0
    out["cause_primary"] = ""
    ...
    for i, row in out.iterrows():
        ...
        a_type = specific_anomaly_label(
            variable=variable,
            residual=residual,
            zscore=z,
            robust_zscore=rz,
            flag_tail=bool(row.get("flag_tail", False)),
            flag_sigma=bool(row.get("flag_sigma", False)),
            flag_robust=bool(row.get("flag_robust", False)),
            flag_iforest=bool(row.get("flag_iforest", False)),
            delta_prev=delta_prev,
            diff_std=diff_std,
        )
        primary = a_type
        out.at[i, "anomaly_type_tr"] = a_type
        ...
        if bool(row.get("flag_iforest", False)):
            details.append("çok değişkenli ayrışma sinyali (iforest)")
        ...
        if local_hint:
            out.at[i, "local_climate_hint"] = local_hint
            details.append(f"yerel_iklim_ipucu={local_hint}")

        news_hits = match_news_catalog(
            ts=ts,
            variable=variable,
            freq=plan.freq,
        ...
        if not news_hits.empty:
            out.at[i, "news_headline_match"] = " | ".join(news_hits["headline"].astype(str).tolist())
            out.at[i, "news_source_match"] = " | ".join(news_hits["source"].astype(str).tolist())
            out.at[i, "news_url_match"] = " | ".join(news_hits["url"].astype(str).tolist())
            out.at[i, "news_hazard_match"] = " | ".join(news_hits["hazard_type"].astype(str).tolist())
            out.at[i, "news_match_score"] = float(news_hits["news_score"].iloc[0])
            out.at[i, "news_match_count"] = int(len(news_hits))
            details.append("haber arşivi eşleşmesi var")
            details.append(f"haber_skoru={float(news_hits['news_score'].iloc[0]):.2f}")
        else:
            out.at[i, "news_headline_match"] = "doğrudan eşleşme yok"
            out.at[i, "news_source_match"] = ""
            out.at[i, "news_url_match"] = ""
            out.at[i, "news_hazard_match"] = ""
            out.at[i, "news_match_score"] = np.nan
            out.at[i, "news_match_count"] = 0
...
def build_global_anomaly_context_report(all_anoms: pd.DataFrame, report_md: Path, top_csv: Path) -> None:
    ...
    for i, r in top.iterrows():
        ...
        lh = str(r.get("local_climate_hint", ""))
        if lh:
            lines.append(f"- Yerel iklim ipucu: {lh}")
        nh = str(r.get("news_headline_match", ""))
        ns = str(r.get("news_source_match", ""))
        nu = str(r.get("news_url_match", ""))
        try:
            nscore = float(r.get("news_match_score", np.nan))
        except Exception:
            nscore = np.nan
        if nh and nh != "doğrudan eşleşme yok":
            lines.append(f"- Haber başlığı eşleşmesi: {nh}")
            if ns:
                lines.append(f"- Haber kaynağı: {ns}")
            if np.isfinite(nscore):
                lines.append(f"- Haber eşleşme skoru: {nscore:.2f}")
            if nu:
                lines.append(f"- Haber linki: {nu}")
        else:
            lines.append("- Haber başlığı eşleşmesi: doğrudan eşleşme bulunamadı")
...
def plot_forecast(out_df: pd.DataFrame, anom_df: pd.DataFrame, last_ds: pd.Timestamp, variable: str, plan: FrequencyPlan, chart_path: Path) -> None:
    ...
    if has_forecast:
        ...
        # anomalies_only modunda grafikte yalnızca gözlem serisini öne çıkar.
        if "actual" in hist.columns and hist["actual"].notna().any():
            ax.plot(hist["ds"], hist["actual"], color="#1f77b4", linewidth=1.2, label="Gerçek gözlem (QC)")
        elif "yhat" in hist.columns and hist["yhat"].notna().any():
            ax.plot(hist["ds"], hist["yhat"], color="#1f77b4", linewidth=1.2, label="Seri")
    ...
    trend_parts = [pd.DataFrame({"ds": hist["ds"], "v": hist["actual"]})]
    if has_forecast:
        trend_parts.append(pd.DataFrame({"ds": fc["ds"], "v": fc["yhat"]}))
    trend_src = pd.concat(trend_parts, ignore_index=True)
    ...
    if has_forecast:
        ...
        ax.set_title(f"YENI MODEL - Kuant Rejim Tahmini - {variable_tr(variable)} ({frequency_tr(plan.freq)})")
    else:
        ax.set_title(f"YENI MODEL - Tarihsel Gözlem + Anomali - {variable_tr(variable)} ({frequency_tr(plan.freq)})")
    ax.set_xlabel("Tarih")
    ...
    if anom_df.empty:
        ...
        for _, r in show.iterrows():
            ...
            news_m = str(r.get("news_headline_match", "")).strip()
            if news_m and news_m != "doğrudan eşleşme yok":
                note_lines.append(f" Haber: {news_m.split('|')[0].strip()}")
...
def plot_regime_probs(ds_fc: pd.Series, fc_probs: np.ndarray, prob_path: Path) -> None:
    ...
    if fc_probs.size == 0:
        ...
        ax.set_ylim(0, 1)
        ax.set_title("YENI MODEL - Tahmin Rejim Olasılıkları")
        ax.set_xlabel("Tarih")
...
def main() -> None:
    ...
    analysis_mode = str(args.analysis_mode).lower().strip()
    if analysis_mode not in {"full", "anomalies_only"}:
        analysis_mode = "full"
    dense_window_mode = str(args.dense_window_mode).lower().strip()
    if dense_window_mode not in {"auto", "off"}:
        dense_window_mode = "auto"

    raw = read_table(args.observations)
    obs, input_kind = normalize_observations(raw, args)
    obs, physical_qc_stats = sanitize_observations_physical(obs)

    if str(args.history_start).strip():
        hs = pd.Timestamp(str(args.history_start).strip())
        obs = obs[obs["timestamp"] >= hs].copy()
    if str(args.history_end).strip():
        he = pd.Timestamp(str(args.history_end).strip())
        obs = obs[obs["timestamp"] <= he].copy()
    if obs.empty:
        raise SystemExit(
            f"Tarih filtresi sonrası kayıt kalmadı: start={args.history_start!s} end={args.history_end!s}"
        )
    ...
    news_catalog_path = resolve_news_catalog_path(args.news_catalog)
    news_catalog = load_news_catalog(news_catalog_path)

    print(f"Girdi: {args.observations} | tür={input_kind} | satır={len(obs)} | değişkenler={','.join(vars_use)}")
    print(f"Tarih filtresi: start={args.history_start or 'yok'} end={args.history_end or 'yok'}")
    print(
        "Fiziksel QC: "
        f"pressure+700={physical_qc_stats.get('pressure_shift_plus700_rows', 0)} | "
        f"drop_humidity={physical_qc_stats.get('humidity_dropped_out_of_range', 0)} | "
        f"drop_precip={physical_qc_stats.get('precip_dropped_out_of_range', 0)} | "
        f"drop_pressure={physical_qc_stats.get('pressure_dropped_out_of_range', 0)} | "
        f"drop_temp={physical_qc_stats.get('temp_dropped_out_of_range', 0)}"
    )
    print(f"Dense window mode: {dense_window_mode}")
    if news_catalog_path is not None:
        print(
            f"Haber kataloğu: {news_catalog_path} | kayıt={len(news_catalog)} | pencere_gün={int(args.news_window_days)}"
        )
    elif args.news_catalog is not None:
        print(f"Haber kataloğu bulunamadı: {args.news_catalog}")
    else:
        print("Haber kataloğu: bulunamadı (eşleşme pas geçildi)")

    index_rows = []
    all_anomaly_rows: list[pd.DataFrame] = []

    for variable in vars_use:
        if dense_window_mode == "off":
            obs_var = obs[obs["variable"] == variable].copy()
            if obs_var.empty:
                continue
            ts_min = pd.to_datetime(obs_var["timestamp"]).min()
            ts_max = pd.to_datetime(obs_var["timestamp"]).max()
            dense_start = pd.Timestamp(ts_min) if pd.notna(ts_min) else None
            dense_end = pd.Timestamp(ts_max) if pd.notna(ts_max) else None
            dense_meta = {
                "threshold": 0,
                "run_years": int(pd.to_datetime(obs_var["timestamp"]).dt.year.nunique()),
                "run_points": int(len(obs_var)),
            }
            if dense_start is not None and dense_end is not None:
                print(
                    f"[{variable}] tam veri modu: {dense_start.date()} -> {dense_end.date()} "
                    f"(puan={dense_meta.get('run_points', 0)})"
                )
        else:
            dense_start, dense_end, dense_meta = infer_dense_window_bounds(obs, variable=variable, ok_value=args.qc_ok_value)
            obs_var = obs[obs["variable"] == variable].copy()
            if dense_start is not None:
                obs_var = obs_var[obs_var["timestamp"] >= dense_start]
            if dense_end is not None:
                obs_var = obs_var[obs_var["timestamp"] <= dense_end]
            if obs_var.empty:
                obs_var = obs[obs["variable"] == variable].copy()

            if dense_start is not None and dense_end is not None:
                print(
                    f"[{variable}] etkin pencere: {dense_start.date()} -> {dense_end.date()} "
                    f"(eşik={dense_meta.get('threshold', 0)}, puan={dense_meta.get('run_points', 0)})"
                )

        plan = choose_frequency_plan(obs_var, variable=variable, ok_value=args.qc_ok_value)
        s = aggregate_series(obs_var, variable=variable, plan=plan, ok_value=args.qc_ok_value)
        if s.empty:
            continue
        ...
        horizon = horizon_steps(last_ds, target_year=args.target_year, freq=plan.freq) if analysis_mode == "full" else 0
        ...
        if analysis_mode == "full":
            ...
            climate_delta_fc = np.zeros(len(fds), dtype=float)
        yhat_fc = apply_bounds(yhat_fc, variable)
        ...
        base_strategy = f"yeni_model_quant_markov_ar_boostblend_analog_{fit['vol_model_used']}_{fit['boost_cfg']}"
        ...
        target_tag = str(args.target_year) if analysis_mode == "full" else f"history_to_{last_ds.year}"
        fc_csv = fc_dir / f"{variable}_{freq_tag}_quant_to_{target_tag}.csv"
        fc_pq = fc_dir / f"{variable}_{freq_tag}_quant_to_{target_tag}.parquet"
        ...
        anom = enrich_anomalies_with_causes(
            ...
            plan=plan,
            news_catalog=news_catalog,
            news_window_days=int(args.news_window_days),
        )
        an_csv = an_dir / f"{variable}_{freq_tag}_anomalies_to_{target_tag}.csv"
        anom["anomalies_csv"] = str(an_csv)
        anom.to_csv(an_csv, index=False)
        if not anom.empty:
            all_anomaly_rows.append(anom.copy())

        chart_png = ch_dir / f"{variable}_{freq_tag}_quant_to_{target_tag}.png"
        prob_png = ch_dir / f"{variable}_{freq_tag}_regime_probs_to_{target_tag}.png"
        ...
        rep = {
            ...
            "dense_window_start": str(dense_start.date()) if dense_start is not None else "",
            "dense_window_end": str(dense_end.date()) if dense_end is not None else "",
            "dense_year_threshold": int(dense_meta.get("threshold", 0)),
            "dense_window_years": int(dense_meta.get("run_years", 0)),
            "dense_window_points": int(dense_meta.get("run_points", 0)),
            "news_catalog_path": str(news_catalog_path) if news_catalog_path is not None else "",
            "news_catalog_rows": int(len(news_catalog)),
            "news_window_days": int(args.news_window_days),
        ...
        rep_json = rep_dir / f"{variable}_{freq_tag}_quant_report_to_{target_tag}.json"
    ...
    index_tag = f"to_{args.target_year}" if analysis_mode == "full" else "history_only"
    idx_csv = out / f"quant_index_{index_tag}.csv"
    idx_pq = out / f"quant_index_{index_tag}.parquet"
    ...
    all_anom_csv = an_dir / f"all_variables_anomalies_{index_tag}.csv"
    all_anom.sort_values(["variable", "ds"], kind="stable").to_csv(all_anom_csv, index=False)
