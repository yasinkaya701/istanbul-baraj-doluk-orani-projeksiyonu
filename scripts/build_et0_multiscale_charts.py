...
def build_monthly(monthly: pd.DataFrame, out_png: Path) -> None:
    ...
    ax.set_title("Aylik ET0 Serisi (1995-2004)", fontsize=19, pad=16)
...
def build_yearly(yearly: pd.DataFrame, out_png: Path) -> None:
    ...
    ax.set_title("Yillik ET0 ve 5 Yillik Ortalama", fontsize=19, pad=16)
...
def build_ten_year_summary(monthly: pd.DataFrame, out_png: Path) -> None:
    df = monthly.copy().sort_values("date")
    df["date"] = pd.to_datetime(df["date"])
    ...
    ax.set_title("10 Yillik Ozet: Ortalama Aylik ET0 Deseni (1995-2004)", fontsize=19, pad=16)
    ax.set_ylabel("ET0 (mm/ay)", fontsize=12)
...
def main() -> None:
    ...
    out1 = args.out_dir / "tarim_et0_monthly_5y_mean.png"
    out2 = args.out_dir / "tarim_et0_yearly_5y_mean.png"
    out3 = args.out_dir / "tarim_et0_10year_summary.png"
    build_monthly(monthly, out1)
    build_yearly(yearly, out2)
    build_ten_year_summary(monthly, out3)
    print(f"Wrote: {out1}")
    print(f"Wrote: {out2}")
    print(f"Wrote: {out3}")
